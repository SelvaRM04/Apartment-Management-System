...
class SecuritiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create security" do
    assert_difference("Security.count") do
      post securities_url, params: { security: { email: @security.email, name: @security.name } }
    end
  ...
  test "should update security" do
    patch security_url(@security), params: { security: { email: @security.email, name: @security.name } }
